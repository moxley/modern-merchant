...
class PaypalBase
{
    ...
    public static function wsdlForEnvironment($environment)
    {
        return $environment == 'live' ? LIVE_WSDL : SANDBOX_WSDL;
    }
}
...
{   
     ...
     * @access public
     */
    public static function registerSoapClient($environment)
    {
        $clientOptions = array("soap_version" => SOAP_1_1, "exceptions" => true, "trace" => true, "connection_timeout " => API_CONNECTION_TIMEOUT);

        $environment = $environment ? $environment : (API_IS_LIVE ? 'live' : 'sandbox');
        $wsdl = PayPalBase::wsdlForEnvironment($environment);
        PayPalBase::setSoapClient(new SoapClient($wsdl, $clientOptions));
    }
...
{   
     ...
     * @access public
     */
    public function prepare($Username, $Password, $Signature, $Subject='', $environment=null)
    {
       PayPalRegistrar::registerSoapClient($environment);
